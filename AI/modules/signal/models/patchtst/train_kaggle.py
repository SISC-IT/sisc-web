...
"""
PatchTST 학습 스크립트 - Kaggle 크론잡 버전
-----------------------------------------------
[train.py와의 차이점]
...
# 서버 크론잡은 trigger_training.py에서 이 모듈을 Kaggle 커널로 push한다.
# ─────────────────────────────────────────────────────────────────────────────
...
CONFIG = {
    ...
    # 서버 크론잡 다운로드 경로: AI/data/weights/patchtst/
...
def build_sequences(full_df: pd.DataFrame, scaler: MinMaxScaler, fit_scaler: bool = True):
    ...
    if missing:
        raise ValueError(f"필수 피처 누락: {sorted(missing)}")
...
def train():
    ...
    if len(tickers) < 2:
        raise ValueError(f"train/val 분할을 위한 ticker가 부족합니다. 현재 {len(tickers)}개")
